<script context="module">export { Controller } from './ctx';
</script>
...
if (process.env.IS_DEV_MODE) {
    if (!itemHeight)
        throw new Error('VirtualList should have itemHeight prop specified to work');
}
...
async function onScroll(e) {
    ...
    if (pageLeft > 0 && pageLeft !== pages[0])
        preloadPage((pages[0] = pageLeft), pages);
    if (pageRight !== pages[1])
        preloadPage((pages[1] = pageRight), pages);
    // }
