<script>import VirtualList from './../../../ui/VirtualList/svelte';
...
function filter(searchTerm) {
    ...
    return COUNTRIES.filter(([code, name]) => code.toLowerCase().includes(value) || name.toLowerCase().includes(value)).sort((a, b) => {
        if (a[0].toLowerCase() === value)
            return -1;
        if (b[0].toLowerCase() === value)
            return 1;
        return 0;
    });
}
