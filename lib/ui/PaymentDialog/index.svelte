...
<script lang="ts">
  ...
  $: isNotCanceled = !(subscription === null || subscription === void 0
    ? void 0
    : subscription.cancelAtPeriodEnd) // TODO: make customer data accesible via context
  ...
  function onSubmit({ currentTarget }) {
    ...
    buyPlan(plan, $stripe, StripeCard, data, savedCard)
      .then((data) => {
        closeDialog()
