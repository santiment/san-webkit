...
import { createCardToken } from './utils.js';
...
export async function start3DSPaymentFlow(stripe, variables) {
    if (!stripe)
        return Promise.reject('Stripe not found');
    const clientSecret = await mutateCreateStripeSetupIntent();
    if (!clientSecret)
    ...
    return mutateSubscribe({
        planId: +planId,
        cardToken: cardToken === null || cardToken === void 0 ? void 0 : cardToken.id,
        coupon,
        paymentMethodId: setupIntent.payment_method,
    }).then(async (sub) => {
        var _a;
        if (!((_a = sub.paymentIntent) === null || _a === void 0 ? void 0 : _a.clientSecret))
            return sub;
        // Check if the PaymentIntent requires any actions and, if so, let Stripe.js
        // handle the flow. If using an API version older than "2019-02-11"
        // instead check for: `paymentIntent.status === "requires_source_action"`.
