...
<nav class="column fluid body-1 {className}" use:adjustHeight>
  <slot {user} {onHelpClick} linkClass="link-BjTLHT" dividerClass="divider-OanOXC">
    <section class="links column gap-l">
      {#each MOBILE_MENU_LINKS as { title, link }}
        {@const href = isFullLink ? getFullLink(link) : link}
        <a {href} on:click={window.__onLinkClick} class="link-BjTLHT">
          {title}
        </a>
      {/each}
    </section>

    <div class="divider-OanOXC fluid" />

    <button class="btn-0 link-BjTLHT" on:click={onHelpClick}>Help & Feedback</button>
