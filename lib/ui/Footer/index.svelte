    ...
    <div class="body-2 row top justify c-waterloo">
      ...
      <div class="forms">
        <div class="digest">
          ...
          <DigestForm class="mrg-xl mrg--b s-162nwoh" label="Subscribe" />
        </div>
      </div>
    </div>
...
:global(body.phone) :global(.s-162nwoh), :global(body.phone-xs) :global(.s-162nwoh) {
  margin-bottom: 40px;
}
...
:global(body.desktop) :global(.s-162nwoh) {
  font: var(--body-3);
}

:global(body:not(.desktop)) :global(.s-162nwoh) {
  width: auto;
}</style>
