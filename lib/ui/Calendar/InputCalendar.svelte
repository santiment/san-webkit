...
function parseInputData(input) {
  ...
  return [parsedInput, parsedInput.map(([day, month, year]) => new Date(`${month || 1}/${day || 1}/20${year || 23}`))];
}
...
function onKeyDown(e) {
  ...
  } else if (+checkIsValidNumber(key) ^ (BlockingNeighbourChar[charBeforeCaret] && BlockingNeighbourChar[charAfterCaret])) {
    return;
  }
