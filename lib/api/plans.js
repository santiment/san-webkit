...
export const queryProductPlans = (productGetter, plansFilter = keepNonDeprecatedPlans) =>
  queryPlans().then(productGetter).then(plansFilter)
const getSanbasePlans = (products) => {
  var _a
  return (_a = products.find(checkIsSanbaseProduct)) === null || _a === void 0 ? void 0 : _a.plans
}
...
export const queryCoupon = (coupon) =>
  query(COUPON_QUERY(coupon), couponOptions).then(couponAccessor)
// -------------------------------------
...
export const mutateSubscribe = (cardToken, planId, coupon) =>
  mutate(SUBSCRIBE_MUTATION(cardToken, planId, coupon)).then(subscribeAccessor)
