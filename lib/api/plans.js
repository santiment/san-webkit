...
import { keepNonDeprecatedPlans, checkIsSanbaseProduct, nonDeprecatedFilter, checkIsIndividualPlan, checkIsBusinessPlan, checkIsSanApiProduct, } from './../utils/plans.js';
...
const filterPlans = (plans, ...selectors) => plans.filter((p) => selectors.every((s) => s(p)));
const findProductPlans = (products, selector) => { var _a, _b; return (_b = (_a = products.find(selector)) === null || _a === void 0 ? void 0 : _a.plans) !== null && _b !== void 0 ? _b : []; };
export const getIndividualPlans = (products) => filterPlans(findProductPlans(products, checkIsSanbaseProduct), nonDeprecatedFilter, checkIsIndividualPlan);
export const getBusinessPlans = (products) => filterPlans(findProductPlans(products, checkIsSanApiProduct), nonDeprecatedFilter, checkIsBusinessPlan);
