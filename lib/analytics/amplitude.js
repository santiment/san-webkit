...
export function updateAmplitude(user_id, name, email) {
    if (process.env.IS_DEV_MODE)
        return;
    if (!window.amplitude)
        return;
    ...
    window.identifyAmplitude = (clb) => {
        var _a;
        clb(identity);
        (_a = window.amplitude) === null || _a === void 0 ? void 0 : _a.identify(identity);
    };
}
