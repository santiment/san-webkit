...
export const isTrackingEnabled = BROWSER && (process.env.IS_DEV_MODE || (process.env.IS_PROD_BACKEND ? canTrackBrowser() : true));
...
const event = isTrackingEnabled
    ? (action, _a = {}, trackers) => {
        ...
        if (trackers.includes(Tracker.SAN)) {
            trackSanEvent(action, new Date(date), normalizeData(Object.assign({ event_category: category, event_label: label }, rest))).catch(() => { });
        }
        if (BROWSER) {
            (_b = window.__trackLegacyWebkitEvent) === null || _b === void 0 ? void 0 : _b.call(window, action, Object.assign({ category, label }, rest));
        }
        return date;
    }
    : noop;
