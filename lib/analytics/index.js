...
const event = isTrackingEnabled
  ? (action, _a = {}, trackers) => {
      var { category, label } = _a,
        rest = __rest(_a, ['category', 'label'])
      if (trackers === void 0) {
      ...
      if (process.env.IS_DEV_MODE) {
        ...
        console.log(
          ...
          normalizeData(Object.assign({ event: action, category, label }, rest)),
        )
        return
      }
      const date = Date.now()
      ...
      if (trackers.includes(Tracker.GA) && window.gtag) {
        window.gtag(
          'event',
          action,
          normalizeData(Object.assign({ event_category: category, event_label: label }, rest)),
        )
      }
