...
export function createConfig({ corsOrigin = 'https://santiment.net', sveltekit = _sveltekit, bulletshell, } = {}) {
    const corsHostname = new URL(corsOrigin).hostname;
    const SENTRY_AUTH_TOKEN = process.env.SENTRY_AUTH_TOKEN;
    const isSentryEnabled = IS_DEV_MODE === false && SENTRY_AUTH_TOKEN;
    return defineConfig({
        plugins: [mkcert(), WebkitSvg(), sveltekit(), bulletshell && BulletshellPlugin()].filter(Boolean),
        build: {
