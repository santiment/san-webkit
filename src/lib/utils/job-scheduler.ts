...
export function JobScheduler() {
  ...
  const scheduler = {
    runNext() {
      ...
      if (job) this.run(job, false)
    },

    run(job: TJob, isForced = false) {
      ...
      if (isForced) queue = queue.filter((item) => item !== job)
    },
  ...
  return {
    schedule(fn: TJob['fn'], id?: number | string, priority?: number) {
      ...
      const job: TJob = { fn, id, priority, scheduled: true, forceTimer: null }
      let index: number
