...
import { SPRITES_OPTIONS, processSvgWithOutput } from '../scripts/svg.js';
...
import { fetchMetricsRestrictions, replaceDefaultMetricsRestrictionsSource, } from '../scripts/metrics-restrictions/index.js';
    ...
    return {
        ...
        async buildStart() {
            if (isLibPackage) {
                ...
                await Promise.all(copyTargets.map((dir) => {
    ...
    return {
        ...
        transform(src, id) {
            if (id.includes('preload-helper')) {
                return {
                    code: src.replace('if (isCss) {', `if (!isCss) {
...
export async function AstroSvelteCtxPlugin(svelteCtxPath) {
    return {
        name: 'astro-svelte-ctx',
        enforce: 'pre',
        transform(src, id) {
            if (id.includes(svelteCtxPath)) {
                const code = src.replace(/(import .*\n)+/g, `import { component_root, pop, push } from 'svelte';
            $&
            push({}, true, () => {});
            component_root(() => {
                     `) + '});';
            ...
            if (id.includes('transitions/router.js') || // Used in build mode
            ...
            if (isSvelteIndexImport) {
                ...
                const code = src.replace(/(export {\s*createContext)/g, `
