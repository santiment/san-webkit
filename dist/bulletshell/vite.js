...
export function BulletshellPlugin({ enabled = false }) {
    const virtualModuleId = 'virtual:bulletshell';
    const resolvedVirtualModuleId = '\0' + virtualModuleId;
    const detectedBulletshellRoutes = new Set();
    if (enabled) {
    ...
    return {
        ...
        transform(code, id) {
            if (id.includes('@sveltejs/kit/src/runtime/server/page/render.js')) {
                return {
                    code: code.replace(`else if (state.prerendering) {
					head += `, `else if (state.prerendering || process.env.BULLETSHELL) {
					head += `),
                };
            }
            if (id.includes('+page.server') && code.includes('_bulletshell')) {
                const sveltekitRouteId = id.slice(id.indexOf('routes/') + 6, id.lastIndexOf('/+page')) || '/';
        ...
        closeBundle() {
            ...
            if (this.environment.name !== 'ssr')
                return;
            return import(SERVER_MANIFEST).then(async ({ manifest }) => {
                ...
                return fg([`${SERVER_CHUNKS_PATH}/hooks.server-*.js`]).then(([entry]) => {
                    ...
                    const hooksFile = hooksFileSource.replace(/const BULLETSHELL_MANIFEST = .+;/, `const BULLETSHELL_MANIFEST = {${bulletshell_routes
                        .map((item) => {
