...
import { AFFILIATLY_PROXY_ROUTE, TRACKING_QUERY_KEYS } from '../../analytics/affiliatly/index.js';
...
const LEGACY_AFFILIATE_MAP = {
    twitter: '2',
};
function extractParams(url, keys, prefix = '') {
    const result = {};
...
export async function callAffiliatly(payload) {
...
export function parseAffiliateCookie(cookieValue) {
...
async function serializeAffiliateCookie(response, cookies) {
    if (!response?.ok)
        return null;
...
async function handleUserTracking({ url, cookies, request }) {
...
async function handleConversionTracking({ url, cookies, request }) {
    if (request.method !== 'POST')
        return new Response(null, { status: 405 });
    if (request.headers.get('origin') !== url.origin)
        return new Response(null, { status: 403 });
    ...
    await callAffiliatly(new URLSearchParams({
        mode: 'mark',
        id_affiliatly: AFFILIATLY_PROGRAM_ID,
...
export const affiliatlyTrackHandle = ({ event, resolve }) => {
