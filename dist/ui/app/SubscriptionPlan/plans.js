...
export const getSubscriptionPlanKey = (planName) => checkIsCustomPlan(planName) ? SubscriptionPlan.CUSTOM.key : planName;
export function convertSubscriptionPlan(planName) {
    const key = getSubscriptionPlanKey(planName);
    if (key in SubscriptionPlan)
        return key;
    return null;
}
...
const CONSUMER_PLANS_ORDER = [
    SubscriptionPlan.FREE.key,
    SubscriptionPlan.PRO.key,
    SubscriptionPlan.MAX.key,
];
const BUSINESS_PLANS_ORDER = [
    SubscriptionPlan.BUSINESS_PRO.key,
    SubscriptionPlan.BUSINESS_MAX.key,
    SubscriptionPlan.CUSTOM.key,
];
const PLANS_UPGRADE_ORDER = [...CONSUMER_PLANS_ORDER, ...BUSINESS_PLANS_ORDER];
export const CONSUMER_PLANS = new Set(CONSUMER_PLANS_ORDER);
export const BUSINESS_PLANS = new Set(BUSINESS_PLANS_ORDER);
export function checkIsTrialEligiblePlan(planKey) {
    return planKey === SubscriptionPlan.PRO.key;
}
export function getNextUpgradePlan(plan) {
