import { BUSINESS_PLANS, CONSUMER_PLANS, checkIsTrialEligiblePlan, convertSubscriptionPlan, getSubscriptionPlanKey, Product, SubscriptionPlan, SubscriptionPlanDetails, } from './plans.js';
...
export function getPlansApiLimitsMap(products) {
    ...
    products.forEach(({ id, plans }) => {
        ...
        plans.forEach(({ name, apiCallLimits }) => {
            ...
            if (!isConsumerBasicPlan && !isBusinessBasicPlan)
                return;
            if (isBusinessBasicPlan !== isSanApi || isConsumerBasicPlan !== isSanbase)
                return;
