<script lang="ts">import { trackEvent } from "../../../analytics/index.js";
...
const { onPlanButtonClick, onPlanChangeClick, onBillingPeriodChangeClick } = useSubscriptionPlanButtonCtx.get();
...
let isSameProductPlan = $derived(
  isConsumerPlan && customer.$.isConsumerSubscription || isBusinessPlan && customer.$.isBusinessSubscription
);
...
function trackPress(e, plan2) {
  trackEvent("press", {
    action: e.currentTarget.textContent?.trim() || "",
    type: e.currentTarget.dataset.type,
    plan: plan2?.name,
    billing: plan2?.interval,
    source
  });
}
</script>
