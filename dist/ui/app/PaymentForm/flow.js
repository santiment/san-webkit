...
import { useStripeCtx } from '../../../ctx/stripe.js';
...
export function usePaymentFlow() {
    ...
    async function processPayment({ paymentMethod, plan, cardToken, }) {
        const stripe = stripeLoader.$;
        if (!stripe)
            return;
        ...
        if (!setupIntentClientSecret)
            return;
        const { setupIntent, error: confirmError } = await stripe.confirmCardSetup(setupIntentClientSecret, { payment_method: paymentMethod }, { handleActions: false });
        if (confirmError) {
