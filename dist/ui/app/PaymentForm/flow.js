...
export function usePaymentFlow() {
    ...
    async function processPayment({ method, action, plan, cardToken, ...paymentData }) {
        const stripe = stripeLoader.$;
        if (!stripe)
            return;
        ...
        return mutateSubscribe(Query)({
            ...
            if (subscription?.paymentIntent?.id) {
                trackAffiliatlyPayment(subscription.paymentIntent.id, Math.ceil((subscription.plan?.amount ?? plan.amount) / 100));
            }
            return subscription;
        })
            .catch((error) => {
