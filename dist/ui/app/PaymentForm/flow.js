...
import { trackAffiliatlyPayment } from '../../../analytics/affiliatly/index.js';
...
export const usePaymentFlowCtx = createCtx('usePaymentFlowCtx', ({ ensureTarget, subscribeMutation, collectAnalytics, onPaymentSuccess, }) => {
    ...
    async function processPayment({ method, action, target = ensureTarget(), cardToken, ...paymentData }) {
        if (!target)
            return;
        const stripe = stripeLoader.$;
        if (!stripe)
...
export function useSanbasePaymentFlow() {
    ...
    usePaymentFlowCtx.set({
        ...
        subscribeMutation: (target, data) => mutateSubscribe(Query)({
            ...data,
        ...
        onPaymentSuccess(target, subscription) {
            ...
            if (subscription?.paymentIntent?.id) {
                trackAffiliatlyPayment(subscription.paymentIntent.id, Math.ceil((subscription.plan?.amount ?? target.amount) / 100));
            }
        },
        collectAnalytics(target) {
            const isEligibleForSanbaseTrial = subscriptionPlan.$.formatted?.isTrialSupported && customer.$.isEligibleForSanbaseTrial;
            return {
                plan: target.name,
                plan_id: target.id,
