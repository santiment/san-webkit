...
const if_expression = math.typed('if', {
    'Timeseries | number, Timeseries | number, Timeseries | number': applyTimeseriesConditionalOperation,
});
...
//  ifFalse: number,
//): number
function applyTimeseriesConditionalOperation(condition, ifTrue, ifFalse) {
    let alignedIfTrue = ifTrue instanceof Timeseries ? alignTimeseries(ifTrue, condition)[0] : ifTrue;
    let alignedIfFalse = ifFalse instanceof Timeseries ? alignTimeseries(ifFalse, condition)[0] : ifFalse;
    ...
    const getIndexValue = (target, i) => target instanceof Timeseries ? target.values[i] : target;
    return new Timeseries(condition.values.map((v, i) => {
