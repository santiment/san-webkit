 ...
 */
export function calculatePerformanceSeries(priceSeries, signalSeries, transactionCost = 0.001, stopLossPercentage, takeProfitPercentage) {
    ...
    const returnsWithStopLossAndProfit = stopLossPercentage || takeProfitPercentage
        ? applyStopLossProtection(strategyReturns, signalSeries, stopLossPercentage, takeProfitPercentage)
        : strategyReturns;
    // Apply transaction costs based on position changes
    const returnsAfterCosts = applyTransactionCosts(returnsWithStopLossAndProfit, signalSeries, transactionCost);
...
function calculateDailyReturns(priceSeries) {
    ...
    return MathOperators.subtract(MathOperators.divide(priceSeries, MathModifiers.shift(priceSeries, 1)), 1);
}
/**
 * Applies trading signals to daily returns with 1-day execution lag
 */
function applyTradingSignals(dailyReturns, signalSeries) {
 ...
 */
function applyTransactionCosts(strategyReturns, signalSeries, transactionCost) {
 ...
 */
function calculateCumulativePerformance(returnsSeries, signalSeries) {
 ...
 */
function createPositionFilter(returnsSeries, _signalSeries) {
    ...
    returnsSeries.values.map((value) => (value > 0 ? 1 : 0)), returnsSeries.timestamps);
 ...
 */
function calculateCumulativeReturns(returnsSeries) {
 ...
 */
function applyStopLossProtection(strategyReturns, signalSeries, stopLossPercentage = 0, takeProfitPercentage = 0) {
    ...
    function applyEarlyExitRule(exitReturn, i) {
