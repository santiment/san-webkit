...
export const Tour = {
    ...
    async start(id, steps, config = {}) {
        ...
        activeDriver = driver({
            allowClose: false,
            smoothScroll: true,
            steps: steps.map(({ element, id, content, completeLabel, onPopoverRender: _, ...popover }, i) => ({
                element,
                popover,
            ...
            onPopoverRender(popover, { driver, state }) {
                ...
                const index = state.activeStep?.data?.index;
...
async function importDriver() {
    return createDriver
        ...
        : Promise.all([import('driver.js'), import('./driver.css')]).then(([{ driver }]) => (createDriver = driver));
}
