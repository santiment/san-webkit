...
selector = null, interval, pane = 0, unit, style = 'line', lineStyle, color = '#00ff00', visible = true, lastValueVisible = true, priceLineVisible = true, crosshairMarkerVisible = true, scaleId, scaleMargins, scaleInverted = false, scaleVisible = true, isSelectorLocked = false, isFilledGradient, transformData, baseline, meta, ...rest }, helpers) {
    const scale = $state({
...
export const processDataStoreScope = (dataStore) => ({
    ...dataStore.params,
    type: dataStore.type,
});
export const useMetricSeriesCtx = createCtx('webkit_useMetricSeriesCtx', (defaultMetrics = [], helpers) => {
    ...
    const asScope = $derived(series.map((item) => ({
        ...
        dataStore: item.type === MetricType.DATA_STORE
