...
import { FORMULA_WARNING, } from '../types.js';
...
export async function fetchFormulaMetric(formula, index, ctx) {
    if (ctx.isCancelled) {
    ...
    function createFetchJob(variable, metric) {
        return new Promise((resolve, reject) => {
            ...
            const dataRequest = () => (metric.formula
                ...
                : queryGenericMetric(metric.dataStore ? { dataStore: metric.dataStore } : { metric: metric.name }, {
                    ...ctx.parameters,
                    version,
