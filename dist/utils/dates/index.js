 ...
 */
export const calculateDaysTo = (target, start = Date.now()) => Math.ceil(((typeof target === 'string' ? Date.parse(target) : +target) - start) / ONE_DAY_IN_MS);
export function setDayEnd(date, { utc = false } = {}) {
    const UTC = utc ? 'UTC' : '';
    date[`set${UTC}Hours`](23, 59, 59, 0);
    return date;
}
export function setDayStart(date, { utc = false } = {}) {
    const UTC = utc ? 'UTC' : '';
    date[`set${UTC}Hours`](0, 0, 1, 0);
    return date;
}
export const getTodaysEnd = (options) => setDayEnd(new Date(), options);
...
export function getFormattedMonthDayYear(date, options = {}) {
    const { MMM, D, YYYY } = getDateFormats(date, options);
    return `${MMM} ${D}, ${YYYY}`;
}
export function getFormattedDetailedTimestamp(datetime, options) {
...
export function getFormattedDayMonthYear(datetime, options) {
    const date = datetime instanceof Date ? datetime : new Date(datetime);
...
export function parseRangeString(range) {
    const amount = parseInt(range, 10);
    return {
        amount,
        modifier: range.slice(amount.toString().length),
    };
}
...
export function formatTimestampToRangeString(timestamp) {
...
export const parseAsStartEndDate = (date, options) => (options.dayStart ? setDayStart : setDayEnd)(parseDate(date), options);
export function suggestPeriodInterval(from, to) {
...
export function normalizeRangeIntervals(ranges, minInterval) {
    if (!minInterval) {
        return ranges;
    }
...
export function getRangeMilliseconds(range) {
    const { amount, modifier } = parseRangeString(range);
    return amount * RangeFormatToTimestamp[modifier];
}
export function normalizeRangeInterval(range, minInterval) {
    return getRangeMilliseconds(range) > getRangeMilliseconds(minInterval) ? range : minInterval;
}
export { TimeZones } from './timezone.js';
