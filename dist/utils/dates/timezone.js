...
].reduce((acc, [timeZone, offset, cities, dst]) => {
...
function calculateTimeZoneUtcOffset(timeZone) {
    const diff = +new Date(new Date().toLocaleString('en', { timeZone })) -
        +new Date(new Date().toLocaleString('en', { timeZone: 'UTC' }));
    return diff / 1000 / 60;
}
function getTimeZoneUtcLabel(offset) {
    if (offset === 0)
        return 'UTC';
