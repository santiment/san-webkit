...
export function log({ executor, cached, schema, mocked, result, }) {
    if (process.env.IS_LOGGING_ENABLED) {
        ...
        console.log(`%c[DEV ONLY] [${executorName}] New query ${state}`, 'background:#478FFF;color:black;padding:3px;border-radius:4px', typeof schema === 'string' ? { schema } : schema, BROWSER ? data : data && typeof data === 'object' ? data.constructor.name : data);
    }
}
