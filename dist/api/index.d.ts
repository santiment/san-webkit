...
export declare function Fetcher<Data, SchemaCreator extends TGqlSchemaCreator>(schemaCreator: SchemaCreator, mapData?: (data: any) => Data, globalOptions?: TExecutorOptions): <GExecutor extends TQueryExecutor = <T>(schema: TGqlSchema, options?: Partial<{
    refCount?: boolean;
    map: (data: unknown) => T;
}>) => Observable<T | T[]>>(executorConfig?: GExecutor | TExecutorConfig<GExecutor>) => (...args: Parameters<SchemaCreator>) => GExecutor extends (...args: any[]) => Observable<any> ? Observable<Data> : Promise<Data>;
export declare const Mutation: <Data, SchemaCreator extends TGqlSchemaCreator>(schemaCreator: SchemaCreator, mapData?: (data: any) => Data) => <GExecutor extends TQueryExecutor = <T>(schema: TGqlSchema, options?: Partial<{
    refCount?: boolean;
    map: (data: unknown) => T;
}>) => Observable<T | T[]>>(executorConfig?: GExecutor | TExecutorConfig<GExecutor>) => (...args: Parameters<SchemaCreator>) => GExecutor extends (...args: any[]) => Observable<any> ? Observable<Data> : Promise<Data>;
