...
export const mutateUpdateUserSettings = ApiMutation((isNightMode) => `mutation {
    updateUserSettings(settings: { theme: "${isNightMode ? 'nightmode' : ''}" }) {
      theme
    }
  }`);
export const useUiCtx = createCtx('useUiCtx', ({ isLiteVersion = false, isNightMode = false, timeZone = 'UTC' } = {}) => {
