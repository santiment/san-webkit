...
import { getBulletshellFileHeaders, type TBulletshellManager } from './utils.js'
...
export function serve(
  ...
  if (opts.gzip && acceptedEncoding.includes('gzip')) files.unshift(filepath + '.gz')
  if (opts.brotli && /(br|brotli)/i.test(acceptedEncoding)) files.unshift(filepath + '.br')
